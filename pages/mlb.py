...
UMPIRE_DATABASE = {
    ...
    "Hitter Friendly (Tight Zone)": ["Pat Hoberg", "Quinn Wolcott", "Andy Fletcher", "Dan Bellino", "Lance Barrett", "CB Bucknor", "Ted Barrett", "Hunter Wendelstedt", "Mark Carlson", "Jeff Nelson", "Rob Drake", "Brian O'Nora", "Todd Tichenor", "Chad Fairchild", "Marvin Hudson", "Mike Muchlinski", "Adam Hamari", "Manny Gonzalez", "D.J. Reyburn"],
}
...
def get_pitcher_and_manager_stats():
    ...
    try:
        ...
        for rec in season_resp['stats'][0]['splits']:
            ...
            pitcher_db[pid] = {'Name': name, 'Out%': shrunk_out, 'K%': shrunk_k, 'BB%': shrunk_bb, 'Hand': hand_map.get(pid, 'R'), 'BF_per_Start': bf_per_start, 'GS': gs, 'IsGhost': tbf < 10}
...
def run_monte_carlo(sp_name, base_out_rate, k_rate, bb_rate, opp_data, park, temp, wind_speed, wind_direction, stadium_azimuth, umpire, manager_shift, pitch_budget, pitches_per_batter, starts_count, num_sims=10000):
    ...
    if effective_wind >= 8.0: adj_out_rate += 0.015
    elif effective_wind <= -8.0: adj_out_rate -= 0.020; adj_bf -= 0.5
...
for game in games:
    ...
    with st.expander(f"⚾ {away_team} @ {home_team} | {temp:.0f}°F 💨{wind_speed:.0f}mph"):
        ...
        for side, sp_name, sp_id, team_abbr, opp_abbr in [(col1, a_sp_name, a_sp_id, away_team, home_team), (col2, h_sp_name, h_sp_id, home_team, away_team)]:
            with side:
                ...
                res = run_monte_carlo(sp_name, lp['actual_out_rate'], lp['actual_k_rate'], lp['actual_bb_rate'], opp_data, home_team, temp, wind_speed, wind_dir, azimuth, "Neutral", MANAGER_DB.get(team_abbr, 0.0), lp['pitch_budget'], lp['pitches_per_batter'], lp['starts_count'])
                ...
                h_prob = float(np.sum(out_sims > line_val)) / 10000 * 100
                l_prob = 100.0 - h_prob
                ...
                st.info(f"📊 Median: {np.median(out_sims):.1f} | Projected BF: {res['adj_bf']:.1f}")
                if st.button(f"💾 Log Result", key=f"log_{sp_id}"):
                    pick = "HIGHER" if h_prob > l_prob else "LOWER"
                    log_play_to_csv(target_date_str, sp_name, team_abbr, opp_abbr, line_val, pick, max(h_prob, l_prob), max(h_ev, l_ev), np.median(out_sims), res['adj_bf'])
