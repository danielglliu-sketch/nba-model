...
st.sidebar.markdown("---")
st.sidebar.caption("v2: Sabermetric xH | Pitch Count | K% | Rest/Fatigue | Wind Vectors")
...
def get_live_pitcher_profile(player_id, fallback: dict, target_date_str: str) -> dict:
    ...
    try:
        ...
        if fallback.get('IsGhost', False) and len(starts) >= 2:
            ...
            if tbf > 0:
                # Sabermetric Expected Hits
                bip = tbf - bb - so
                expected_h = bip * 0.290
                
                profile['actual_out_rate'] = (tbf - bb - expected_h) / tbf
        ...
        if len(starts) >= 3:
            ...
            if r_tbf > 0:
                # Sabermetric Expected Hits for recent form
                r_bip = r_tbf - r_bb - r_so
                expected_r_h = r_bip * 0.290
                
                recent_out_rate = (r_tbf - r_bb - expected_r_h) / r_tbf
...
def run_monte_carlo(
    ...
    # Ace Override logic
    ACE_LIST = ["Zac Gallen", "Logan Gilbert", "Zack Wheeler", "Corbin Burnes", "Gerrit Cole"]
    if sp_name in ACE_LIST:
        pitch_budget = max(pitch_budget, 100.0)

    # ── Pitch-count BF estimate ────────────────────────────────────────────────
    pitch_count_bf = pitch_budget / max(pitches_per_batter - 0.25, 3.3)
    
    # FIX: Widen the hard cap so Inning Eaters aren't trapped
    hard_cap_bf    = pitch_count_bf + 6.0   

    # Blended BF projection
    if starts_count < 5:
        # FIX: Heavily trust the Pitch Count model for April, ignore March averages
        pc_w, sa_w, ms_w = 0.70, 0.05, 0.25   
    else:
    ...
    if abs(k_bonus) > 0.004:
        factors.append(f"⚡ K-Rate ({k_rate*100:.1f}%): {'+' if k_bonus>0 else ''}{k_bonus*100:.1f}% out-rate adj")
    ...
    if adj_bf > hard_cap_bf:
        factors.append(f"🚨 Pitch Count Cap: BF limited to {hard_cap_bf:.1f} ({pitch_budget:.0f}-pitch budget)")
        adj_bf = hard_cap_bf
...
st.markdown(
    ...
    "Career Blending · Decoupled BF Variance · Sabermetric xH_"
)
    ...
    with st.expander(label):
        ...
        for side, sp_name, sp_id, team_abbr, opp_abbr in [
            ...
            with side:
                ...
                with oc1: o_odds = st.number_input("Over Odds:", value=-110, step=5, key=f"oo_{key_base}")
                with oc2: u_odds = st.number_input("Under Odds:", value=-110, step=5, key=f"uo_{key_base}")
