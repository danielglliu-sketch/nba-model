...
# ─────────────────────────────────────────────────────────────────────────────
# 1. THE 5-ON-5 STARTING ROSTER DATABASE (Player vs Player Ratings)
# ─────────────────────────────────────────────────────────────────────────────
STARTING_FIVES = {
    'BOS': {
...
# Failsafe: For teams not yet mapped above, use league average so the math never crashes
DEFAULT_ROSTER = {
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. LIVE ESPN FETCHERS (Ensures Accurate Daily News & Standings)
# ─────────────────────────────────────────────────────────────────────────────
...
def is_active(player_name, team_injuries):
    """Checks the live ESPN injury report to see if the specific player is ruled out."""
    last_name = player_name.split()[-1]
    return not any(last_name in inj for inj in team_injuries)

# ─────────────────────────────────────────────────────────────────────────────
# 3. PLAYER-VS-PLAYER (5-ON-5) PREDICTION ENGINE
# ─────────────────────────────────────────────────────────────────────────────
def predict_game(h, a, standings, injuries):
    ...
    # 1. Base Team Factors
    total += 3.5
    factors.append({"icon": "🏠", "name": "Home Court", "adj": 3.5, "why": "Standard home venue advantage."})
    ...
    # 2. --- PLAYER VS PLAYER MATCHUP ENGINE ---
    SCALING_FACTOR = 0.15 # Scales raw 1-100 rating diffs into realistic win % points
    ...
    for pos in positions:
        ...
        # Live Injury Scanner: If either player is hurt today, void the matchup logic
        ...
        # The Player vs Player Math
        # Points generated by Home player against Away player's defense
        h_score = max(0, hp['off'] - ap['def'])
        # Points generated by Away player against Home player's defense
        a_score = max(0, ap['off'] - hp['def'])
        
        # The net edge converted to a win probability shift
    ...
    # 3. Overall Team Injury & Motivation Logic
    if h_inj: total -= 4.0; factors.append({"icon": "🤕", "name": "Injury Penalty", "adj": -4.0, "why": f"{h} missing rotation players."})
    if a_inj: total += 4.0; factors.append({"icon": "🤕", "name": "Opponent Injury", "adj": 4.0, "why": f"{a} missing rotation players."})

    if h_std['win_pct'] < 0.36 and h_std['wins'] + h_std['losses'] > 15:
        total -= 8.0; factors.append({"icon": "🎯", "name": "Tanking Penalty", "adj": -8.0, "why": f"{h} prioritizing draft odds."})
    elif a_std['win_pct'] < 0.36 and a_std['wins'] + a_std['losses'] > 15:
        total += 8.0; factors.append({"icon": "🎯", "name": "Tanking Boost", "adj": 8.0, "why": f"{a} prioritizing draft odds."})
...
# 4. USER INTERFACE
# ─────────────────────────────────────────────────────────────────────────────
st.title("🏀 Live NBA Predictor (Player vs Player Edition)")
st.divider()
...
for game in slate:
    ...
    with st.expander(header):
        ...
        # --- PLAYER VS PLAYER MATCHUP BOARD ---
        if pred['matchups']:
