...
# --- PAGE SETUP ---
st.set_page_config(page_title="Ultimate Sports AI 2026", page_icon="🏆", layout="wide")
...
# Create the Tabs at the very top of the page
tab_nba, tab_mlb = st.tabs(["🏀 NBA Predictor", "⚾ MLB Predictor"])

# ═════════════════════════════════════════════════════════════════════════════
# 🏀 NBA MASTER AI LOGIC & DATA
# ═════════════════════════════════════════════════════════════════════════════
NBA_CLEARED_PLAYERS = ["Joel Embiid", "Tyrese Maxey"]

NBA_BACKUP_STANDINGS = {
    'DET': {'wins': 56, 'losses': 21, 'record': '56-21', 'win_pct': 0.727, 'home_record': '30-9', 'away_record': '26-12'},
    'BOS': {'wins': 52, 'losses': 25, 'record': '52-25', 'win_pct': 0.675, 'home_record': '26-11', 'away_record': '26-14'},
    'NYK': {'wins': 50, 'losses': 28, 'record': '50-28', 'win_pct': 0.641, 'home_record': '28-9', 'away_record': '22-19'},
    'PHI': {'wins': 43, 'losses': 34, 'record': '43-34', 'win_pct': 0.558, 'home_record': '22-17', 'away_record': '21-17'},
...
NBA_ELIMINATED_TEAMS = ['MIL', 'CHI', 'IND', 'BKN', 'WAS', 'MEM', 'NOP', 'DAL', 'UTA', 'SAC']
...
def get_nba_slate():
    ...
    try:
        ...
        for event in data.get('events', []):
            ...
            away = next((c for c in comp['competitors'] if c['homeAway'] == 'away'), None)
            if home and away:
                games.append({'h': norm_nba(home['team']['abbreviation']), 'a': norm_nba(away['team']['abbreviation']),
                              'h_name': home['team']['displayName'], 'a_name': away['team']['displayName']})
        if games: return games
    except: pass
    return []
...
def get_nba_standings():
    ...
    try:
        ...
        for conf in data.get('children', []):
            for entry in conf.get('standings', {}).get('entries', []):
                abbr = norm_nba(entry['team']['abbreviation'])
                stats = {s.get('name', '').lower(): s for s in entry.get('stats', [])}
                def get_rec(k): return stats.get(k, {}).get('summary') or stats.get(k, {}).get('displayValue', '0-0')
                wins, losses = int(stats.get('wins', {}).get('value', 0)), int(stats.get('losses', {}).get('value', 0))
                home_rec = get_rec('home')
...
def get_nba_b2b():
    ...
    try:
        ...
        data = requests.get(url, timeout=5).json()
        for event in data.get('events', []):
            for c in event['competitions'][0]['competitors']: b2b.add(norm_nba(c['team']['abbreviation']))
    except: pass
    return b2b

def predict_nba(h, a, standings, injuries, b2b_set):
    h_td = NBA_TEAM_DATA.get(h, {'off_rtg': 112, 'def_rtg': 115})
    a_td = NBA_TEAM_DATA.get(a, {'off_rtg': 112, 'def_rtg': 115})
    h_std = standings.get(h, {'wins': 0, 'losses': 0, 'record': '0-0', 'win_pct': 0.5, 'home_record': '0-0', 'away_record': '0-0'})
    a_std = standings.get(a, {'wins': 0, 'losses': 0, 'record': '0-0', 'win_pct': 0.5, 'home_record': '0-0', 'away_record': '0-0'})
    factors, total = [], 0.0
...
with tab_nba:
    ...
    nba_slate, nba_standings, nba_injuries, nba_b2b = get_nba_slate(), get_nba_standings(), get_nba_injuries(), get_nba_b2b()

    if not nba_slate:
        st.info("No NBA games scheduled for today, or ESPN hasn't published the slate yet.")
    else:
        for game in nba_slate:
            pred = predict_nba(game['h'], game['a'], nba_standings, nba_injuries, nba_b2b)
            with st.expander(f"{game['h_name']} vs {game['a_name']} | Winner: {pred['winner']} ({pred['conf']:.1f}%)"):
                st.markdown(f"### 🏆 {pred['winner']} Wins")
                for f in pred['factors']:
                    color = "#28a745" if f['adj'] > 0 else "#dc3545" if f['adj'] < 0 else "#888888"
